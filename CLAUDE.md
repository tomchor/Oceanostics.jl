...
- **`SubFilterKineticEnergyEquation`**: Sub-filter KE budget terms — `subfilter_kinetic_energy` (Kˢ = ½τⁱⁱ, a composite `AbstractOperation`) and `SubFilterKineticEnergyDissipationRate` (εˢ = filter(ε) − εˡ, a `KernelFunctionOperation` that wraps that op, à la `KineticEnergyCrossScaleFlux`). Built on `FilteredKineticEnergyEquation` and `KineticEnergyEquation`, so it is included after both.
