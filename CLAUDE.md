...
- **`SpatialFilters`** (submodule): Spatial filters (`box_filter.jl`, `gaussian_filter.jl`) for diagnostics that need scale separation. Every 1D kernel sizes its in-range check with `stencil_length(grid, d, ψ)` (the operand's own extent, `N+1` for a `Face` location along a `Bounded` direction) rather than `size(grid, d)`; the recursive (fused) methods reach the operand through `fargs[end]`. Using the cell count there silently mistreats the last face of a `Face`-located operand (`:shrink` drops its own weight, `:edge` clamps to the face below), which surfaced as `NaN`s when filtering the model's `diffusive_flux_z` KFO with a degenerate identity-scale Gaussian. The staged multi-dimensional path (`_stage_into_temp`) must `fill_halo_regions!` each intermediate: skipping it is correct on CPU but on GPU the next pass reads unwritten memory and loses stencil weight (~60% of the signal at wide stencils, growing with the number of intermediates). Only half-width 1 survives unpatched — precisely where the identity-scale filters in the filtered/sub-filter suites sit, which is why CPU CI never caught it. Correctness additionally needs `halo ≥ stencil half-width` per filtered direction.
