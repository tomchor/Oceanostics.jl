...
viscosity. Both are non-negative for a down-gradient closure: ``\tau_{ij}`` is the flux, so it
carries the opposite sign to the stress, exactly as in
[the momentum equation](momentum_equation.md).
