...
```math
\partial_t c^2 = 2c\,\partial_t c = -2c\,\partial_j(u_j c) - 2c\,\partial_j q^c_j + 2c\,F^c
```

where ``q^c_j`` is the diffusive flux of ``c`` in the ``j``-th direction and ``F^c``
is any applied forcing, in the convention [the tracer equation](tracer_equation.md) sets out. This
budget can be further decomposed into a diffusive transport term and a dissipation rate:

```math
-2c\,\partial_j q^c_j = -\partial_j(2c\,q^c_j) + 2\,\partial_j c \cdot q^c_j
```

The second term, ``\chi = -2\,\partial_j c \cdot q^c_j``, is the tracer variance
