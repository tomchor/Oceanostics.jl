...
for Fickian diffusion), and ``F^c`` represents any external forcing applied to the tracer. This is the
same flux convention [the potential energy equation](potential_energy_equation.md) uses for the
buoyancy flux ``q_j``.
