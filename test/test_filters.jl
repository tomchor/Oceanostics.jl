...
@testset "Filters" begin
    @testset "BoxFilter" begin
        ...
        @testset "Linear field is unchanged on interior (1D, 2D, 3D)" begin
            ...
            # exactly on cells whose stencil does not reach into halos (where the linear
            # function is not truly periodic).
            c = CenterField(grid)
            set!(c, (x, y, z) -> x + 2y + 3z)
            fill_halo_regions!(c)

            # Away from any filtered dimension's boundary, a width=1 stencil stays in the interior.
        ...
        @testset "Averaging matches an explicit stencil sum" begin
            ...
            # Reference: explicit 1D average in x with width = 1
            cf = Field(BoxFilter(c; dims=(1,), width=1))
            compute!(cf)

            ref = similar(interior(c))
            for i in 1:Nx, j in 1:Ny, k in 1:Nz
                ip = i + Hx; jp = j + Hy; kp = k + Hz
                ref[i, j, k] = (ci[ip-1, jp, kp] + ci[ip, jp, kp] + ci[ip+1, jp, kp]) / 3
            end
            @test interior(cf) ≈ ref

            # Reference: explicit 3D average with width = 1 (27-point box)
            cf3 = Field(BoxFilter(c; dims=(1, 2, 3), width=1))
            compute!(cf3)

            ref3 = similar(interior(c))
            for i in 1:Nx, j in 1:Ny, k in 1:Nz
                ip = i + Hx; jp = j + Hy; kp = k + Hz
                s = 0.0
                for di in -1:1, dj in -1:1, dk in -1:1
                    s += ci[ip+di, jp+dj, kp+dk]
                end
                ref3[i, j, k] = s / 27
            end
        ...
        @testset "Accepts AbstractOperation as input" begin
            ...
            f = Field(bf)
            compute!(f)
            # Linear in x + y: filter preserves values on interior cells whose stencil stays inside.
        ...
        @testset "Accepts another KernelFunctionOperation as input" begin
            ...
            # Build a trivial KFO via BoxFilter itself, then filter again.
        ...
        @testset "Halo validation" begin
            # grid halo is (2, 2, 2); width = 3 must fail for any selected dim.
            c = CenterField(grid)
