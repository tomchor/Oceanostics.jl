...
function test_u_momentum_hfs_budget_closure(grid)
    ...
    # hydrostatic_free_surface_u_velocity_tendency. `momentum_advection = Centered()` is required
    # so the diagnostic Advection (which wraps U_dot_∇u with the model's scheme) matches the
