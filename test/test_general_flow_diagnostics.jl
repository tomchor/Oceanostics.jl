...
@testset "General flow diagnostics on $(typeof(arch).name.wrapper)" begin
    ...
    @testset "SubfilterFlux" begin
        ...
        @test all(interior(Field(SubfilterFlux(c, b, filt; loc))) .≈ interior(τ_hand))
        ...
        @test all(interior(Field(SubfilterFlux(u, c, filt; loc))) .≈ interior(τx_hand))

        # subfilter momentum-stress special case
        vᶜ = Field(@at loc v)
        τxy_hand = Field(Field(filt(Field(uᶜ * vᶜ))) - Field(filt(uᶜ)) * Field(filt(vᶜ)))
        @test all(interior(Field(SubfilterFlux(u, v, filt; loc))) .≈ interior(τxy_hand))
        ...
        τ_uniform = Field(SubfilterFlux(uniform_model.velocities.u, uniform_model.tracers.c, filt; loc))
